...
# Fixed structure outline prompt - 3-level hierarchy
FIXED_OUTLINE_PROMPT = """Create a {sizesections}-section outline about {keyword} for {articleaudience} readers.
...
For each subsection, provide {sizeparagraphs} specific paragraph points (numbered 1, 2, 3...) that will become individual paragraphs.
Aim for subsections that work well as tables or lists.
Return only the outline in <outline> tags."""

# Variable paragraphs outline prompt - 3-level hierarchy
VARIABLE_PARAGRAPHS_OUTLINE_PROMPT = """Create a {sizesections}-section outline about {keyword} for {articleaudience} readers.
Focus on practical, actionable content. Use Roman numerals (I, II, III) for main sections and letters (A, B, C) for {sizeheadings} subsections each.
For each subsection, provide 1-5 specific paragraph points (numbered 1, 2, 3...) based on content complexity and natural flow.
Aim for subsections that work well as tables or lists.
Return only the outline in <outline> tags."""

# 2-level hierarchy outline prompt (when subsections=0)
TWO_LEVEL_OUTLINE_PROMPT = """Create a {sizesections}-section outline about {keyword} for {articleaudience} readers.
...
# Variable paragraphs 2-level hierarchy
VARIABLE_TWO_LEVEL_OUTLINE_PROMPT = """Create a {sizesections}-section outline about {keyword} for {articleaudience} readers.
Focus on practical, actionable content. Use Roman numerals (I, II, III) for main sections.
For each section, provide 1-5 specific paragraph points (numbered 1, 2, 3...) based on content complexity and natural flow.
Aim for content that works well as tables or lists.
Return only the outline in <outline> tags."""
...
PARAGRAPH_GENERATE_PROMPT = """Write paragraph {current_paragraph} of {paragraphs_per_section} for section "{heading}".
...
# Simplified paragraph with heading - From 51 lines to 10 lines
PARAGRAPH_PROMPT = """Write paragraph {current_paragraph} of {paragraphs_per_section} for section "{heading}".
Target {articleaudience} readers in {articlelanguage} with {voicetone} tone.
