...
from utils.rate_limiter import unsplash_rate_limiter,pexels_rate_limiter,openverse_rate_limiter,pixabay_rate_limiter,hugging_face_rate_limiter
...
try:
    ...
    logger.warning("PIL/Pillow not installed. Image compression will be disabled.")
    PIL_AVAILABLE = False
...
def get_image_list_unsplash(
    ...
    logger.info(f"Fetching {num_images} images for keyword: {keyword}")
    ...
    try:
        logger.debug(f"Sending request to Unsplash API with params: {params}")
        
        # Use rate limiter if available
        if unsplash_rate_limiter:
            logger.debug("Using rate limiter for Unsplash API call")
            ...
            data = unsplash_rate_limiter.execute_with_rate_limit(make_api_call)
        else:
            response = requests.get(url, params=params, headers=headers)
            response.raise_for_status()
            data = response.json()
        ...
        logger.debug(f"Found {total_results} total results")
        ...
        for result in data.get("results", []):
            image_data = {
                ...
                "alt": result["alt_description"] or keyword,
                "photographer": result["user"]["name"],
                "photographer_url": result["user"]["links"]["html"]
            }
            images.append(image_data)
            logger.debug(f"Processed image {result['id']} by {result['user']['name']}")
            
        logger.success(f"Successfully fetched {len(images)} images")
        return images
...
def generate_ai_image_list(
    ...
    """
    if not config.enable_image_generation:
        logger.debug("AI image generation is disabled.")
        return []

    if not num_images or num_images <= 0:
        logger.debug("No valid num_images specified. Returning empty list.")
        return []

    folder = 'ai_images'
    clean_and_prepare_folder(folder)

    logger.info(f"Generating {num_images} AI images for keyword: {keyword}")
    ...
    for i in range(num_images):
        try:
            ...
            logger.debug(f"Sending request to Hugging Face with prompt: {prompt}")
            ...
            if not image_data:
                logger.warning("No image data returned from Hugging Face.")
                continue
            ...
            logger.debug(f"Generated image saved as {ai_file}")

            count += 1

        except Exception as e:
            logger.error(f"Error generating AI image {i}: {str(e)}")

    logger.info(f"Successfully generated {len(images)} AI images.")
    return images
...
def get_image_list_openverse(
    ...
    logger.info(f"Fetching up to {count} images for keyword: {keyword}")
    ...
    try:
        while len(images) < count:
            logger.debug(f"Sending request to Openverse API with params: {params}")

            # Use rate limiter if available
            if openverse_rate_limiter:
                logger.debug("Using rate limiter for Openverse API call")
                def make_api_call():
                    response = requests.get(url, params=params)
                    response.raise_for_status()
                    return response.json()
                data = openverse_rate_limiter.execute_with_rate_limit(make_api_call)
            else:
                response = requests.get(url, params=params)
                response.raise_for_status()
                data = response.json()
            ...
            logger.debug(f"Found {total_results} total results on page {params['page']}")
            ...
            for result in data.get("results", []):
                ...
                if width == 0 or height == 0:
                    # Dimensions missing; try downloading the image to check
                    try:
                        ...
                        logger.warning(f"Failed to fetch image dimensions for {result['id']}: {str(e)}")
                        continue  # Skip if image download fails
                    
                # Filter out square images
                if width == height:
                    continue
                ...
                image_data = {
                    "id": result["id"],
                    "url": result["url"],
                    "thumb": result.get("thumbnail", result["url"]),
                    "alt": result.get("title", keyword),
            ...
            params["page"] += 1
            if len(images) < count and total_results <= params["page"] * count:
        ...
        logger.error(f"Error fetching Openverse images: {str(e)}")
        return []
...
def get_image_list_pexels(
    ...
    try:
        logger.debug(f"Sending request to Pexels API with params: {params}")
        
          # Use rate limiter if available
        if pexels_rate_limiter:
            logger.debug("Using rate limiter for Pexels API call")
        ...
        data = pexels_rate_limiter.execute_with_rate_limit(make_api_call) if pexels_rate_limiter else make_api_call()

        images = []
        logger.debug(f"Found {len(data.get('photos', []))} results")

        for result in data.get("photos", []):
            image_data = {
                "id": str(result["id"]),
                "url": result["src"]["large"],
                "thumb": result["src"]["tiny"],
                "alt": result.get("alt", keyword),
                "photographer": result["photographer"],
                "photographer_url": result["photographer_url"]
            }
            images.append(image_data)
            logger.debug(f"Processed image {result['id']} by {result['photographer']}")

        logger.success(f"Successfully fetched {len(images)} images")
        return images
    except Exception as e:
        logger.error(f"Error fetching Pexels images: {str(e)}")
        return []
...
def get_image_list_pixabay(
    ...
    try:
        logger.debug(f"Sending request to Pixabay API with params: {params}")
        
          # Use rate limiter if available
        if pixabay_rate_limiter:
            logger.debug("Using rate limiter for Pixabay API call")
        ...
        data = pixabay_rate_limiter.execute_with_rate_limit(make_api_call) if pixabay_rate_limiter else make_api_call()

        images = []
        logger.debug(f"Found {data.get('totalHits', 0)} results")
        ...
        for result in data.get("hits", []):
            image_data = {
                ...
                "alt": result.get("tags", keyword),
                "photographer": result.get("user", "Unknown"),
                "photographer_url": f"https://pixabay.com/users/{result.get('user', '')}-{result.get('user_id', '')}/"
            }
            images.append(image_data)
            logger.debug(f"Processed image {result['id']} by {result.get('user')}")

        logger.success(f"Successfully fetched {len(images)} images")
        return images
    except Exception as e:
        logger.error(f"Error fetching Pixabay images: {str(e)}")
        return []


def download_image(url: str, save_path: str, compress: bool = False, quality: int = 70) -> bool:
    """
    ...
    try:
        ...
        if compress and PIL_AVAILABLE:
            logger.debug(f"Compressing downloaded image with quality: {quality}")
            compress_image(save_path, quality)
            
        logger.success(f"Successfully downloaded and saved image to {save_path}")
        return True
    except Exception as e:
...
def process_body_image(
    ...
    """
    Process an image for article body.
    ...
    """
    logger.debug(f"Processing body image {index} for keyword: {keyword}")
    
    try:
        # Return image data for WordPress
        processed_data = {
            "file": image_data.get("url"),  # This is the regular URL from simplified structure
            "url": image_data.get("url"),   # Same URL for markdown display
        ...
        if not processed_data["file"] or not processed_data["url"]:
            logger.error(f"Missing required URLs for body image {index}")
            return None
            
        logger.debug(f"Processed body image {index} with URL: {processed_data['url']} and alignment: {alignment}")
        return processed_data
        
    except Exception as e:
        logger.error(f"Error processing body image {index}: {str(e)}")
        return None
...
def process_feature_image(
    ...
    Returns:
        Optional[Dict[str, str]]: Processed image data or None
    """
    logger.debug(f"Processing feature image for keyword: {keyword}")
    
    try:
        # Return image data for WordPress
        processed_data = {
            "file": image_data.get("url"),  # This is the regular URL from simplified structure
            "url": image_data.get("url"),   # Same URL for markdown
            "alt": image_data.get("alt") or keyword,
            "caption": f"Featured image by {image_data.get('photographer', 'Unknown')} on {sourceUsed}",
            "photographer": image_data.get("photographer", "Unknown"),
            "photographer_url": image_data.get("photographer_url", ""),
            "alignment": alignment  # Add alignment class
        }
        ...
        if not processed_data["file"] or not processed_data["url"]:
            logger.error("Missing required URLs for feature image")
            return None
            
        logger.debug(f"Processed feature image with URL: {processed_data['url']} and alignment: {alignment}")
        return processed_data
        
    except Exception as e:
        logger.error(f"Error processing feature image: {str(e)}")
        return None
...
def get_image_list_from_stock_sources(
    ...
    if img == "unsplash":
        ...
        logger.warning(f"Unsupported image source: {config.stock_primary_source}")
        sourceUsed = "Unknown"
...
def generate_image_description(
    ...
    logger.info(f"Alt text generation time: {time.time() - start_time:.2f} seconds")
    return alt_text
...
def generate_filename(
    ...
    if os.path.exists(tmp_file):
        try:
            ...
            logger.info(f"Renamed {tmp_file} to {new_file_path}")
            return new_file_path
        except OSError as e:
            logger.error(f"Error renaming file: {e}")
            return tmp_file  
    else:
        logger.error (f"Temporary file {tmp_file} does not exist")
        return tmp_file  
...
def fetch_image(
    ...
    if image_source.lower() == "imageai":
        return generate_ai_image_list(query,config,num_images=10),"AI"

    if primary_source.lower() not in  ["unsplash", "pexels", "pixabay", "openverse"]:
       logger.error("Invalid primary image source")
...
def get_article_images(
    ...
    """
    Get all images needed for an article.
    
    Args:
        keyword (str): Article keyword
        config (ImageConfig): Image configuration settings
        num_images (int, optional): Number of body images
        save_dir (str): Directory to save images
        timestamp (str, optional): Timestamp for unique filenames
    Returns:
        Tuple[Optional[Dict[str, str]], List[Dict[str, str]]]: Feature image and body images
    """
    logger.info(f"Starting image collection for article: {keyword}")
    
        
   
    # Get image list
    total_images = (num_images or config.max_number_of_images) + 1
    logger.debug(f"Requesting {total_images} images (including feature image)")
    
    images,sourceUsed= fetch_image(
                config=config,
                image_source=config.image_source,
                primary_source=config.stock_primary_source,
                query=keyword,
                num_of_image=total_images,
                use_secondary_library=config.secondary_source_image
             )
    ...
    index = image_randomizer(images)
    logger.debug("Processing feature image")
    ...
    # Process body images
    logger.debug(f"Processing {len(unused_images) - 1} body images")
    body_images = []
    
    for i, image in enumerate(unused_images, start=0):
       
        if processed := process_body_image(
            image, 
            keyword, 
            i, 
            sourceUsed,
            ...
            body_images.append(processed)
           
    body_images = randomize_image_selection(body_images)
    logger.success(f"Successfully processed {len(body_images)} body images and 1 feature image")
...
def compress_image(image_path: str, quality: int = 70) -> str:
    ...
    if not PIL_AVAILABLE:
        logger.warning("PIL/Pillow not available. Skipping image compression.")
        return image_path
    ...
    try:
        logger.debug(f"Compressing image: {image_path} with quality {quality}")
        ...
        if compressed_size < original_size:
            ...
            logger.success(f"Image compressed successfully: {reduction:.1f}% reduction ({original_size} -> {compressed_size} bytes)")
            ...
            logger.debug("Compression didn't reduce file size. Keeping original.")
