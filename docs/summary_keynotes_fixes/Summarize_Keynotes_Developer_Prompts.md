# Developer Prompts for Summarize and Keynotes Enhancement

This document provides step-by-step instructions for implementing the enhancements to the Summarize and Keynotes sections of the article generation system. The implementation is divided into three main tasks, each with its own prompt.

## Prompt 1: Configuration and Chunking Utilities

### Task Description
Implement the configuration changes and chunking utilities needed for the enhanced Summarize and Keynotes functionality.

### Implementation Steps

1. Add the following configuration parameters to the `Config` class in both `script1/utils/config.py` and `script2/config.py`:
   ```python
...
2. Update the Config initialization in both `script1/main.py` and `script2/main.py` to include the new parameters:
   ```python
...
3. Create a new file `script1/article_generator/chunking_utils.py` with the following content:
   ```python
...
4. Create a new file `script2/article_generator/chunking_utils.py` with the same content as above, but replace `logger` with `provider` to match script2's logging system.

## Prompt 2: Summary Generation Enhancement

### Task Description
Enhance the summary generation functionality to use a specified model with a large context window and implement chunking for large articles.

### Implementation Steps

1. In `script1/article_generator/content_generator.py`, find the `generate_article_summary` function and replace it with:
   ```python
...
2. In `script2/article_generator/generator.py`, find the section in the `generate_article` method that handles summary generation and replace it with:
   ```python
...
## Prompt 3: Keynotes Generation Enhancement

### Task Description
Enhance the keynotes (block notes) generation functionality to use a specified model with a large context window and implement chunking for large articles.

### Implementation Steps

1. In `script1/article_generator/text_processor.py`, find the `generate_block_notes` function and replace it with:
   ```python
...
2. In `script2/article_generator/text_processor.py`, find the `generate_block_notes` function and replace it with:
   ```python
   @retry(
...
After implementing these changes, you should test the functionality with the following scenarios:

1. **Basic Functionality Test**: Generate an article with the new configuration enabled and verify that the summary and keynotes are generated correctly.

2. **Model Selection Test**: Configure different models for summary and keynotes generation and verify that they are used correctly.

3. **Chunking Test**: Generate an article with a very large content to test the chunking functionality. You can temporarily reduce the chunk size in the configuration to force chunking even for smaller articles.

4. **Error Handling Test**: Test error scenarios, such as invalid model names or API failures, to ensure the system handles them gracefully.

Remember to check the logs to verify that the correct models are being used and that chunking is working as expected when needed.
