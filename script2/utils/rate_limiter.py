...
class RateLimiter:
    ...
    def __init__(self, name: str, config: RateLimitConfig):
        ...
        logger.info(
            f"Rate limiter '{name}' initialized with {config.rpm} RPM and {config.rpd or 'unlimited'} RPD"
        )
    ...
    def _refill_tokens(self) -> None:
        ...
        if minutes_elapsed >= 1:
            self.minute_tokens = min(
                self.config.rpm,
                self.minute_tokens + int(minutes_elapsed * self.config.rpm),
            )
...
def initialize_rate_limiters(
    ...
    pixabay_rate_limiter = RateLimiter("pixabay",openverse_config)
